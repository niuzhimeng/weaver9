 ...
 * 工时控制流程-提交
 */
public class ManHourSubmit extends BaseAction {
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        this.writeLog("工时控制流程-提交 Start requestid=" + requestId + "  operatetype --- " + operateType + "   fromTable --- " + tableName);
        try {
            ...
            sykbgs = ManHourUtil.sub(sykbgs, zcgs);
            spzgs = ManHourUtil.add(spzgs, zcgs);

            recordSet.executeUpdate("update uf_xmcyjl set sykbgs = ?, spzgs = ? where xmbh = ? and xmlx = ? and jdmc = ? and xmcy = ?",
                    sykbgs, spzgs,
                    hdbh, xmlx, jdmc, bgr);

            this.writeLog("工时控制流程-提交 End ===============");
        } catch (Exception e) {
            this.writeLog("工时控制流程-提交 异常： " + e);
            requestInfo.getRequestManager().setMessageid("110000");
            requestInfo.getRequestManager().setMessagecontent("工时控制流程-提交 异常： " + e);
            return "0";
        }
